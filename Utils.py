...
def visualize_result3 (img0, img1, img2, txt=""):

    img0= tf.squeeze(img0)
    ...
    plt.xticks([])
    plt.yticks([])
    plt.title("Cwssim-color loss map")
    plt.imshow(img2)
...
def pad_image(image, pad_size):
    ...
    image = tf.reshape(image, (image.shape[1], image.shape[2], image.shape[3]))
    return image
...
def preprocess_data (patches, expand=False):
    x = np.array(patches)
...
def batch_evaluation (valid_patches, autoencoder, ae_batch_splits):
    x_valid = valid_patches #= preprocess_data(valid_patches)
...
def get_ovr (y_true, y_pred):
    y_true = np.array (y_true, dtype='uint8')
