...
class Metric_win():
    def __init__(self, patch_size=128, window_size=7, pad_size = 0):
    ...
    def cw_ssim_score (self, s1, s2, is_complex, full=False):
        ...
        if (not full):
            ...
            if (int_shape(cwssim)[1] < self.patch_size):
                cwssim = tf.image.resize(cwssim, [self.patch_size,self.patch_size], method='nearest') #TODO restore this line!!!!!!
                #cwssim = tf.image.resize(cwssim, [self.patch_size+6, self.patch_size+6], method='nearest')
